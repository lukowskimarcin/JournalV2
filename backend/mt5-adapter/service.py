...
class MetaTraderService:
    def __init__(self):
        self.login = 1021359  # 1021359
        self.password = "XX3ZetLq"  # "XX3ZetLq"
        self.server = "TradeQuo-Server"
        self.timezone = pytz.timezone("Etc/UTC")

        if not mt5.initialize(
            login=self.login, server=self.server, password=self.password
        ):
    ...
    def history_deals_get(self, from_date, to_date, group="*", position_id=None):
        if position_id is not None:
            history_orders = mt5.history_deals_get(position=position_id)
        else:
